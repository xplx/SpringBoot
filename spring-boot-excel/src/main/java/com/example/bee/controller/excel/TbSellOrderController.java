package com.example.bee.controller.excel;
...
public class TbSellOrderController {
    ...
    public R findOrderInfo(){
        List<TbSellOrder> tbSellOrderList = new ArrayList<>();
        try {
            TbSellOrderExample example = new TbSellOrderExample();
            tbSellOrderList = tbSellOrderMapper.selectByExample(example);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return R.ok().setData(tbSellOrderList);
    }
